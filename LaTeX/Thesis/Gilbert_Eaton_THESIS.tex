...
\newcolumntype{G}{>{\centering\arraybackslash}m{2.8cm}}
\newcolumntype{H}{>{\centering\arraybackslash}m{2.2cm}}
\newcolumntype{I}{>{\centering\arraybackslash}m{4.4cm}}
\newcolumntype{J}{>{\centering\arraybackslash}m{2.5cm}}
...
{0pt}{8.2ex plus 1ex minus .2ex}{2.6ex plus .2ex}
...
\begin{figure}[ht]
	...
	\caption[Example of production line using multi-spectral acquisition system with a robotic arm sorting the objects.]{Example of production line using multi-spectral acquisition system with a robotic arm sorting the objects. \textit{Source: https://www.bkinstruments.co.kr/sub22/menu9\_8\_5.php}}
...
\begin{figure}[h]
	...
	\begin{subfigure}{.5\textwidth}
		...
		\label{fig:Lab}
	\end{subfigure}%
	\caption[Colour spaces (a) HSV and (b) CIE-Lab.]{Colour spaces (a) HSV \textit{Source: https://doc.qt.io/archives/qq/qq26-adaptivecoloring.html} and (b) CIE-Lab \textit{Source: https://www.sciencedirect.com/science-/article/abs/pii/S0044848604004867}.}
...
\begin{figure}[h]
	...
	\begin{subfigure}{.9\textwidth}
		...
		\label{fig:ifra-red}
	\end{subfigure}%
	\caption[(a) Spectral response of human eye (left) and CCD and CMOS sensors (right) and (b) Infrared spectrum breakdown relative to visible.]{(a) Spectral response of human eye (left) and CCD and CMOS sensors (right) \textit{Source: http://www.icpdas.com/products/Vision/mavis/vision\_glossary\_n~z.htm} (b) Infrared spectrum breakdown relative to visible. \textit{Source: https://www.infratec.eu/sensor-division/service-support/glossary/infrared-radiation/}}
	\label{fig:spectrum}
\end{figure}%
...
\begin{figure}[h]
	...
	\caption[Example of a spectroscope taking measurements from a fruit sample.]{Example of a spectroscope taking measurements from a fruit sample \cite{choi}.}
...
\begin{figure}[h]
	...
	\caption[A thermal image apple with bruises showing the Long Wavelength Infrared (LWIR) part of the electromagnetic spectrum. In this range, sensors can detect thermal properties at room temperature.]{A thermal image apple with bruises showing the Long Wavelength Infrared (LWIR) part of the electromagnetic spectrum. In this range, sensors can detect thermal properties at room temperature \cite{baranowski}.}
	\label{fig:thermal}
\end{figure}%
...
\begin{figure}[h]
	...
	\caption[Strawberry picking machine (a) demonstrating end-effector mechanism (b) placing strawberry into the foam tray]{Strawberry picking machine (a) demonstrating end-effector mechanism (b) placing strawberry into the foam tray \cite{hayashi}.}
...
\begin{figure}[h]
	...
	\caption[Pseudo-coloured strawberry heat-map showing the white area of the berry being firmest and the red area softest.]{Pseudo-coloured strawberry heat-map showing the white area of the berry being firmest and the red area softest\cite{elmasry2}.}
...
\begin{figure}[h]
	...
	\caption[Apple sorting system showing the lighting box with cameras in the background with opening bowls in the foreground.]{Apple sorting system showing the lighting box with cameras in the background with opening bowls in the foreground \cite{sofu}.}
...
Machine learning is a branch of computer science and AI that uses an iterative 'learning' approach to building a classifier model. The model's weights are updated by an algorithm that, given any training set ($x_1, y_1$), ($x_2, y_2$), ... , ($x_n, y_n$), can learn a function $y = f(x)$ to make prediction of an unseen $x_{test}$ variable. 

Neural networks (NN) take a different form to traditional machine learning (such as statistical regression and classification models), where information is represented in a graph where the nodes are designed to mimic naturally occurring neurons of the human brain. They are connected with axons (edges), and fire (activate) when the neuron's function threshold is met. A simple NN consists of an input layer constructed from a set of neurons whose input values are based on features derived by humans, one hidden layer that is not interacted with by humans (black box), and an output layer that resolves the final decision based on the former layer's activations. 

Deep learning refers to the depth of the network or number of layers the model has. Using a deeper network allows for multiple convolutions in a CNN and therefore better abstraction, albeit more computationally expensive. A range of methods have been investigated from very basic MLP networks, SVM, and ANN to more complex architectures such as CNNs.
...
\begin{figure}[h]
	...
	\caption[A 2D example of SVM hyperplane separating two classes. The examples on the boundary are considered the support vectors.]{A 2D example of SVM hyperplane separating two classes. The examples on the boundary are considered the support vectors. \textit{Source: https://en.wikipedia.org/wiki/Support-vector\_machine}}
	\label{fig:SVM}
\end{figure}%
...
\begin{figure}[h]
	...
	\caption[An example of a ConvNet pass showing convolutional layers, subsampling (pooling), and finally, fully connected layers in the classification section.]{An example of a ConvNet pass showing convolutional layers, subsampling (pooling), and finally, fully connected layers in the classification section. \textit{Source: https://www.pyimagesearch.com/2014/06/09/get-deep-learning-bandwagon-get-perspective/}}
	\label{fig:conv3}
\end{figure}
...
\begin{figure}[h]
	...
	\caption[MaxPool operation. Left: The input volume, in this example, of (224x224x64) is sliced in depth and pooled individually using \textit{filter size} and \textit{stride} set at two, resulting in a (112x112x64) volume output, keeping in mind the depth does not change. Right: With a \textit{stride} of two, the max is taken from each (2x2) matrix.]{MaxPool operation. Left: The input volume, in this example, of (224x224x64) is sliced in depth and pooled individually using \textit{filter size} and \textit{stride} set at two, resulting in a (112x112x64) volume output, keeping in mind the depth does not change. Right: With a \textit{stride} of two, the max is taken from each (2x2) matrix. \textit{Source: https://github.com/mratsim/Arraymancer/issues/174}}
	\label{fig:maxpool}
\end{figure}

\textit{Rectified Linear Units (ReLU)} - The activation function is responsible for the threshold level at which each neuron 'fires' for positive or negative activations. In the case of ReLU, $f(x) = max(0, x)$ where x is the input to the neuron. This is a step function or \textit{rectified} function, where negative values are set to zero, however positive values can be large, increasing the non-linear decision function and overall network robustness. Other functions can be used, which smooth the step function, such as hyperbolic tangent $f(x) = tanh(x)$ or sigmoid $f(x) = (1+e^{-x})^{-1}$.
...
It can be seen that the structure could be adapted in other ways also, but it is hard to know what the best approach may be without performing testing and analysis, therefore these examples only illustrate possible suggestions. With added layers comes added complexity and time, however the best balance of accuracy and complexity must be found.
...
\begin{figure}[h]
	...
	\caption[Neural network topology with 2 hidden layers.]{Neural network topology with 2 hidden layers. \textit{Source: https://www.gabormelli.com/RKB/Neural\_Network\_Hidden\_Layer}}
...
\begin{figure}[h]
	...
	\caption[Strawberries with dominant specular reflections reducing the ability to analyse pixels.]{Strawberries with dominant specular reflections reducing the ability to analyse pixels. \textit{Source: https://wa-health.kaiserpermanente.org/summer-strawberry-salad/}}
...
The polariser investigation and application is described further in Chapter \ref{sec:paper_1}. 

\begin{figure}[h]
	\centering
	\includegraphics[width=.8\linewidth]{polarization.png}
	\caption[Electromagnetic polarization. A circular or randomly polarized source becomes a single polarity after passing through the material.]{Electromagnetic polarization. A circular or randomly polarized source becomes a single polarity after passing through the material. \textit{Source: http://physicsopenlab.org/2016/03/13/electromagnetic-waves-polarization/}}
...
\begin{figure}[h]
	...
	\caption[Cree CXB3590 power distribution curves for a range of colour temperatures at $T=85^{\circ}C$.]{Cree CXB3590 power distribution curves for a range of colour temperatures at $T=85^{\circ}C$. \textit{Source: https://www.cree.com/led-components/media/documents/ds-CXB3590.pdf}}
...
\begin{figure}[h]
	...
	\caption[Fluctuations recorded for 50Hz nominal AC power signal - European grid (2015) samples of (a) 24 hour period, and (b) 15 minute intervals.]{Fluctuations recorded for 50Hz nominal AC power signal - European grid (2015) samples of (a) 24 hour period, and (b) 15 minute intervals. \textit{Source: https://phys.org/news/2018-01-diverse-frequency-fluctuations-power-grids.html}}
	\label{fig:50hz}
\end{figure}
...
\begin{figure}[h]
	...
	\begin{subfigure}{.45\textwidth}
		\centering
		\includegraphics[width=\linewidth,angle=270,origin=c]{bench_hal_film.png}
		\caption{}
		\label{fig:bench_hal_film}
	\end{subfigure}%
	\begin{subfigure}{.45\textwidth}
		\centering
		\includegraphics[width=\linewidth,angle=270,origin=c]{bench_hal_film2.png}
		\caption{}
...
\begin{figure}[h]
	...
	\begin{subfigure}{.42\textwidth}
		...
		\label{fig:LED_30w}
	\end{subfigure}%
	\caption[(a) Cree\textregistered 72W LED Light Bar, and (b) Cree\textregistered 30W LED Light Bar.]{(a) Cree\textregistered 72W LED Light Bar \textit{Source: http://www.myautolighting.com/index.php?route=product/product\&product\_id=197}, and (b) Cree\textregistered 30W LED Light Bar. \textit{Source: https://www.mydeal.com.au/pair-6inch-30w-led-work-light-bar-flood-reverse-fog-lights-4wd-642788}}
	\label{}
\end{figure} 
...
\begin{figure}[ht]
	...
	\caption[Left to right: (a) HSV colour space showing hue circle and saturation vector, (b) raw RGB image with some background lighting noise, (c) saturation channel with berries segmented after pre-processing, (d) image reduced to berry region showing four small under ripe areas detected by the algorithm.]{Left to right: (a) HSV colour space showing hue circle and saturation vector \textit{Source: https://doc.qt.io/archives/qq/qq26-adaptivecoloring.html}, (b) raw RGB image with some background lighting noise, (c) saturation channel with berries segmented after pre-processing, (d) image reduced to berry region showing four small under ripe areas detected by the algorithm.}
	\label{}
\end{figure}
...
\begin{figure}[h]
	...
	\begin{subfigure}{.32\textwidth}
		\centering
		\includegraphics[width=.9\linewidth,scale=0.5]{thresh/image_0_160.jpg}
		\caption{}
		\label{fig:thresh0_160}
	\end{subfigure}%
	\begin{subfigure}{.32\textwidth}
		\centering
		\includegraphics[width=.9\linewidth,scale=0.5]{thresh/image_1_160.jpg}
		\caption{}
		\label{fig:thresh1_160}
	\end{subfigure}%
	\begin{subfigure}{.32\textwidth}
		\centering
		\includegraphics[width=.9\linewidth,scale=0.5]{thresh/image_2_160.jpg}
		\caption{}
		\label{fig:thresh2_160}
	\end{subfigure}%

	\caption{Images (a) (b), and (c) showing berry reflections on the internal wall of the punnets, and green calyx regions leftover from under-thresholding.}
	\label{}
\end{figure}
...
\begin{figure}[h]
	...
	\begin{subfigure}{.32\textwidth}
		\centering
		\includegraphics[width=.9\linewidth,scale=0.5]{thresh/image_0_180.jpg}
		\caption{}
		\label{fig:thresh0_180}
	\end{subfigure}%
	\begin{subfigure}{.32\textwidth}
		\centering
		\includegraphics[width=.9\linewidth,scale=0.5]{thresh/image_1_180.jpg}
		\caption{}
		\label{fig:thresh1_180}
	\end{subfigure}%
	\begin{subfigure}{.32\textwidth}
		\centering
		\includegraphics[width=.9\linewidth,scale=0.5]{thresh/image_2_180.jpg}
		\caption{}
		\label{fig:thresh2_180}
	\end{subfigure}%

	\caption{Images (a) (b), and (c) showing the optimal threshold level of 180. The small berry reflections remaining can be removed easily using morphology.}
	\label{}
\end{figure}
...
\begin{figure}[h]
	...
	\begin{subfigure}{.32\textwidth}
		\centering
		\includegraphics[width=.9\linewidth,scale=0.5]{thresh/image_0_200.jpg}
		\caption{}
		\label{fig:thresh0_200}
	\end{subfigure}%
	\begin{subfigure}{.32\textwidth}
		\centering
		\includegraphics[width=.9\linewidth,scale=0.5]{thresh/image_1_200.jpg}
		\caption{}
		\label{fig:thresh1_200}
	\end{subfigure}%
	\begin{subfigure}{.32\textwidth}
		\centering
		\includegraphics[width=.9\linewidth,scale=0.5]{thresh/image_2_200.jpg}
		\caption{}
		\label{fig:thresh2_200}
	\end{subfigure}%
	
	\caption{Images (a) (b), and (c) showing the removal of berry flesh pixels after over-thresholding with a value of 200.}
	\label{}
\end{figure}


Pre-processing consists of reducing the image to a Region Of Interest (ROI) in the spacial location where the punnet is consistently captured. 



Under thresholding was found to make the morphology difficult to separate both the green calyx from the red berry flesh as well as the smaller, noisy blobs and berry reflections from actual berries. Figures \ref{fig:thresh0_160}, \ref{fig:thresh1_160}, and \ref{fig:thresh2_160} illustrate a threshold of 160 on the saturation channel showing this problem.


Over thresholding also produced undesired effects having removed some of the berry flesh pixels, as shown in Figures \ref{fig:thresh0_200}, \ref{fig:thresh1_200}, and \ref{fig:thresh2_200} at a level of 200, whilst Figures \ref{fig:thresh0_180}, \ref{fig:thresh1_180}, and \ref{fig:thresh2_180} are at the optimal range of 180. 



The post-processing methods use morphology, removal of small regions (noise reduction), and area calculations to return the affected areas as indicated by the ellipses in Figure \ref{fig:hue_processed}.
...
\begin{algorithm}
	...
	\label{alg:basic}
	\begin{algorithmic}[1]
		\Procedure{AnalyseHSV}{$I_{crop}$, $H_{min}$, $H_{max}$, $A_{max}$}       
		\State $Z \gets \emptyset$
		\Comment{initialise empty return set}
		\State $B_{sat} \gets \emptyset$
		\Comment{initialise empty analysis region}
		\State $(H,S,V) \gets RGBtoHSV(I_{crop})$
		\Comment{predefined colourspace transformation}
		\State $B_{sat} \gets S_{(x, y)}[(x, y) > 128]$
		\For{each $b \in B_{sat}$}
		\If{$b > H_{min}$ and $b < H_{max} $}
		\State $Z \gets Z + b$
		\Comment{accumulate all pixels in range}
		\If{$Z > A_{max}$}
		\State \Return Z
		\Comment{return area if greater than max}
		\EndIf
		\EndIf
		\EndFor
		\State \Return 0
		\Comment{return zero, otherwise}
		\EndProcedure
	\end{algorithmic}
\end{algorithm}
...
\begin{figure}[ht]
	...
	\begin{subfigure}{.5\textwidth}
		\centering
		\includegraphics[width=.9\linewidth]{over_under/under_1.jpg}
		\caption{}
		\label{fig:under_1}
	\end{subfigure}%
	\begin{subfigure}{.5\textwidth}
		\centering
		\includegraphics[width=.9\linewidth]{over_under/under_2.jpg}
		\caption{}
		\label{fig:under_2}
	\end{subfigure}%
	...
	\begin{subfigure}{.5\textwidth}
		\centering
		\includegraphics[width=.9\linewidth]{over_under/under_3.jpg}
		\caption{}
		\label{fig:under_3}
	\end{subfigure}%
	\begin{subfigure}{.5\textwidth}
		\centering
		\includegraphics[width=.9\linewidth]{over_under/under_4.jpg}
		\caption{}
		\label{fig:under_4}
	\end{subfigure}%
...
\begin{figure}[h]
	...
	\begin{subfigure}{.5\textwidth}
		\centering
		\includegraphics[width=.9\linewidth]{over_under/over_1.jpg}
		\caption{}
		\label{fig:over_1}
	\end{subfigure}%
	\begin{subfigure}{.5\textwidth}
		\centering
		\includegraphics[width=.9\linewidth]{over_under/over_4.jpg}
		\caption{}
		\label{fig:over_2}
	\end{subfigure}%
	...
	\begin{subfigure}{.5\textwidth}
		\centering
		\includegraphics[width=.9\linewidth]{over_under/over_3.jpg}
		\caption{}
		\label{fig:over_3}
	\end{subfigure}%
	\begin{subfigure}{.5\textwidth}
		\centering
		\includegraphics[width=.9\linewidth]{over_under/over_2.jpg}
		\caption{}
		\label{fig:over_4}
	\end{subfigure}%
...
\begin{figure}[h]
	...
	\caption[Example of Specular Reflection on an art display. Note the specular reflection interference with the image causing the loss of information and colour.]{Example of Specular Reflection on an art display. Note the specular reflection interference with the image causing the loss of information and colour. \textit{Source: http://vcg.isti.cnr.it/Publications/2011/ABC11/j.1467-8659.2011.01971.x.pdf}}
...
\begin{figure}[h]
	...
	\caption[The interface showing specular reflection of an electromagnetic wave in either the TE or TM orientation.]{The interface showing specular reflection of an electromagnetic wave in either the TE or TM orientation. \textit{Source: https://www.tulane.edu/~sanelson/eens211/proplight.htm}}
...
In order to quantify the results in terms of image processing, an experiment was established in which 50 images were acquired before applying a constant threshold level to each. This was followed by taking an area calculation of each image's threshold region and finally, the average pixel area calculated across all images. Each image could then be graphed in relation to the overall average pixel area to show the differences between images over time. 

The average pixel area was obtained by first taking a threshold of subjects in the each of the frames:
\begin{equation}
R = \sum_{i=1}^{P}t_1 \leq x \leq t_2
\end{equation}

with $P$ denoting the number of pixels in each image and $t_1$, $t_2$ the threshold limits. The average area ($Area_{avg}$) was calculated and is given by the mean definition:
\begin{equation}
...
\begin{table}[h]
	\centering
	\caption{Example images acquired after application of cross-polariser showing removal of specular reflections.}
	\label{tab:specular_examples}
	\begin{tabular}{*6{H}}
		
		\includegraphics[width=\linewidth]{spec/spec_1.jpg} & \includegraphics[width=\linewidth]{spec/spec_2.jpg} & \includegraphics[width=\linewidth]{spec/spec_3.jpg} &         \includegraphics[width=\linewidth]{spec/spec_4.jpg} & \includegraphics[width=\linewidth]{spec/spec_5.jpg} &         \includegraphics[width=\linewidth]{spec/spec_6.jpg} \\
		
		
		\includegraphics[width=\linewidth]{spec/spec_7.jpg} & \includegraphics[width=\linewidth]{spec/spec_8.jpg} & \includegraphics[width=\linewidth]{spec/spec_9.jpg} &         \includegraphics[width=\linewidth]{spec/spec_10.jpg} & \includegraphics[width=\linewidth]{spec/spec_11.jpg} &         \includegraphics[width=\linewidth]{spec/spec_12.jpg} \\
		
		
		\includegraphics[width=\linewidth]{spec/spec_13.jpg} & \includegraphics[width=\linewidth]{spec/spec_14.jpg} & \includegraphics[width=\linewidth]{spec/spec_15.jpg} &         \includegraphics[width=\linewidth]{spec/spec_16.jpg} & \includegraphics[width=\linewidth]{spec/spec_17.jpg} &         \includegraphics[width=\linewidth]{spec/spec_18.jpg} \\
		
		
		\includegraphics[width=\linewidth]{spec/spec_19.jpg} & \includegraphics[width=\linewidth]{spec/spec_20.jpg} & \includegraphics[width=\linewidth]{spec/spec_21.jpg} &         \includegraphics[width=\linewidth]{spec/spec_22.jpg} & \includegraphics[width=\linewidth]{spec/spec_23.jpg} &         \includegraphics[width=\linewidth]{spec/spec_24.jpg} \\
		
	\end{tabular}
\end{table} 

The resulting images taken using the designed enclosure have dramatically improved the image stabilisation when compared to the AC lighting system. The DC-powered lighting system resulted in very small intensity fluctuations, and was measured to be $<0.1\%$ as seen in Figure \ref{fig:image_diff_dc} and \ref{fig:image_diff_dc_2}. 
...
When the polarisers are orthogonal ($90^{\circ}$), the result is a more flattened image that removes the specular reflections almost entirely.  Figure \ref{fig:berries_no_spec} depicts this change and it can be seen that the surface of the strawberries are much clearer and will allow better image processing due to the increase of pixels which can be analysed, and a decrease in noise. Further examples of punnet images with specular reflection removal are displayed in Table \ref{tab:specular_examples}. 
...
\begin{figure}[h!]
	...
	\begin{subfigure}{.6\textwidth}
		\centering
		\includegraphics[width=0.9\linewidth]{system_construct_1.jpg}
		\caption{}
		\label{fig:system_construct_1}
	\end{subfigure}%
	\begin{subfigure}{.45\textwidth}
		\centering
		\includegraphics[width=0.9\linewidth, angle=270]{enclosure_3.jpg}
		\caption{}
...
where one pixel of movement over the distance of the punnet is equivalent to $\frac{1}{1000} = 0.001 = 0.1\%$. This shows that in order to generate sub-pixel motion blur, a shutter speed of $<428\mu s$ is required.
...
Using Equation \ref{eq:shutter}, the motion blur of one pixel is $~0.5ms$ which accounts for a $0.1\%$ difference in relation to the number of pixels, relative to punnet motion in the images. The image displayed in Figure \ref{fig:blur_1} has minor blurring resulting from a $5ms$ shutter speed, and is calculated to have $11.6$ pixels of movement or $1.16\%$ blur. 
...
Experiments revealed that the both sensors had intermittent detection problems where they would not respond well in full-speed conditions. Experiments performed included reflectance confirmation, calibration, independent power supply, and replacing sensor units, without success. As it was concluded that it was the environment creating errors (due to the metal surroundings and close proximity to objects) a redesign would be required to be refit them inside the enclosure. However, a machine vision solution exists to solve this problem, given that the punnets flow in the same direction seen by a fixed camera. A common method of tracking moving objects is to analyse the image differences in a stream. That is:
\begin{equation}
D_{image} = I_{n+1} - I_n
\end{equation}

where the difference image ($D_{image}$) is the result, and $I_n$ the image number in the stream, showing pixels with spatial variance (the moving objects) in an otherwise stationary scene. By analysing the difference regions, the punnet could be centred and acquired accurately, even though an extra time resource was taken up by the additional algorithm. This extra process added to the RGB thread does not impede the image processing algorithm which is the most time consuming element of the application. This is due to the threaded assignment of tasks such that the RGB acquisition thread can wait for a new punnet to be in position (image difference processing), then capture the images required, whilst the image processing thread is assessing the previous punnet. The only constraint is that the image processing is complete before the punnet reaches the eject sensor.
...
Initial testing consisted of experiments to confirm that punnets packed withing a reasonable tolerance of the ideal weight could be pushed from the line, but not agitated so much as to damage the fruit or tip over the punnet as it traversed from the main conveyor down the ejection ramp. Approximately 20 punnets were used to programatically fine-tune the pneumatic burst timing and duration according to when the punnet passed the eject sensor. 

This test concluded that in order to provide enough force to push the $250g-300g$ punnet and successfully transfer it from the production conveyor (running at approximately $27cm/s$) to the exit ramp, required the following:
\begin{itemize}
...
The duration of the burst was found to be the main contributing factor where a burst of $100ms$ would not be enough to change the direction of the punnet adequately, whereas $300ms$ would tip the punnet over and spill the contents, both of which is unacceptable. A range of punnet weights were tested to ensure that over and under weight instances were successfully removed without damage. 
...
The design flowchart in Figure \ref{fig:ejector_logic} shows the interaction between the main thread, image processing thread (where the decision is made), and the microcontroller. The controller must also activate for the pre-determined timing calculated for the pneumatic burst, including delay between sensing and activation and the duration the solenoid is opened. Additionally, a conveyor activation feedback signal was connected to the micro-controller to ensure that any production line stop could be accounted for and added to the ejection timing, giving the system continuity under any circumstance. 
...
%\begin{table}[h]
	...
	\begin{longtable}{*5{G}}
		\toprule
		 & Before & 0 min & 5 min & 10 min \\
	
		$950nm$ &
		\includegraphics[width=\linewidth]{bruise/nobruise_950.jpg} &
		\includegraphics[width=\linewidth]{bruise/bruise_950.jpg} &
		\includegraphics[width=\linewidth]{bruise/bruise5m_950.jpg} &
		\includegraphics[width=\linewidth]{bruise/bruise10m_950.jpg}\\
		
		$1050nm$  &
		\includegraphics[width=\linewidth]{bruise/nobruise_1050.jpg} &
		\includegraphics[width=\linewidth]{bruise/bruise_1050.jpg} &
		\includegraphics[width=\linewidth]{bruise/bruise5m_1050.jpg} &
		\includegraphics[width=\linewidth]{bruise/bruise10m_1050.jpg} \\	 
		
		$1150nm$ &
		\includegraphics[width=\linewidth,angle=90,scale=0.80]{bruise/nobruise_1150.jpg} &  
		\includegraphics[width=\linewidth]{bruise/bruise_1150.jpg} & 
		\includegraphics[width=\linewidth]{bruise/bruise5m_1150.jpg} & 
		\includegraphics[width=\linewidth]{bruise/bruise10m_1150.jpg}\\     
		
		$1250nm$ & 
		\includegraphics[width=\linewidth]{bruise/nobruise_1250.jpg} & 
		\includegraphics[width=\linewidth]{bruise/bruise_1250.jpg} &
		\includegraphics[width=\linewidth]{bruise/bruise5m_1250.jpg} &
		\includegraphics[width=\linewidth]{bruise/bruise10m_1250.jpg} \\
		
		$1350nm$ &
		\includegraphics[width=\linewidth]{bruise/nobruise_1350.jpg} & 
		\includegraphics[width=\linewidth]{bruise/bruise_1350.jpg} &  
		\includegraphics[width=\linewidth]{bruise/bruise5m_1350.jpg} &
		\includegraphics[width=\linewidth]{bruise/bruise10m_1350.jpg} \\
		
		$1450nm$ & 
		\includegraphics[width=\linewidth]{bruise/nobruise_1450.jpg} &
		\includegraphics[width=\linewidth]{bruise/bruise_1450.jpg} &
		\includegraphics[width=\linewidth]{bruise/bruise5m_1450.jpg} &
		\includegraphics[width=\linewidth]{bruise/bruise10m_1450.jpg} \\
		
		$1550nm$ &
		\includegraphics[width=\linewidth]{bruise/nobruise_1550.jpg} &  
		\includegraphics[width=\linewidth]{bruise/bruise_1550.jpg} & 
		\includegraphics[width=\linewidth]{bruise/bruise5m_1550.jpg} & 
		\includegraphics[width=\linewidth]{bruise/bruise10m_1550.jpg} \\
		       
		$1650nm$ &
		\includegraphics[width=\linewidth]{bruise/nobruise_1650.jpg} &
		\includegraphics[width=\linewidth]{bruise/bruise_1650.jpg} &
		\includegraphics[width=\linewidth]{bruise/bruise5m_1650.jpg} &
		\includegraphics[width=\linewidth]{bruise/bruise10m_1650.jpg} \\
...
\begin{figure}[h]
	...
	\begin{subfigure}{.5\textwidth}
		\centering
		\includegraphics[width=.8\linewidth]{bruise_ir_2.png}
		\caption{}
...
\begin{figure}[ht]
	...
	\begin{subfigure}{.47\textwidth}
		\centering
		\includegraphics[width=0.9\linewidth]{myponga_line.jpg}
		\caption{}
		\label{fig:myponga_line}
	\end{subfigure}%
	\begin{subfigure}{.43\textwidth}
		\centering
		\includegraphics[width=0.9\linewidth]{myponga_system_line.jpg}
		\caption{}
...
A dataset of 500+ images, ground-truthed by operators was used to assess the algorithms implemented. Images were graded under the list of following defects which allowed to gain quantitative results for both current and future methods:

\begin{itemize}
	\item Over Ripe - Single instance
	\item Under Ripe - Single instance
	\item Over Ripe - Multiple instance
	\item Under Ripe - Multiple instance
	\item Dry Bruise
	\item Wet Bruise
	\item Soil / Sand
	\item Size Difference
	\item Too Small
	\item Foreign Object
	\item Insect / Pest
	\item Unsure
...
The underripe and overripe defects were broken into single and multiple defect instances in order to develop algorithms in an attempt to  discriminate between punnets with a single defect (which could potentially be passed depending on seasonality), and instances with multiple defected berries. The labels generated are in the form of a text file associated with each image, which may indicate one or many defects for the same punnet. A sample of labelled test images are shown in Table \ref{tab:test_examples} including single and multiple instance underripe and overripe, size difference and small, and pass. 

	
\begin{longtable}{l*3{I}}
		
		A & 
		\includegraphics[width=\linewidth]{tests/under_s_1.jpg} & \includegraphics[width=\linewidth]{tests/under_s_2.jpg} &         \includegraphics[width=\linewidth]{tests/under_s_3.jpg}\\
		
		B & 
		\includegraphics[width=\linewidth]{tests/under_m_1.jpg} & \includegraphics[width=\linewidth]{tests/under_m_2.jpg} &         \includegraphics[width=\linewidth]{tests/under_m_3.jpg} \\
		
		C & 
		\includegraphics[width=\linewidth]{tests/over_s_1.jpg} & \includegraphics[width=\linewidth]{tests/over_s_2.jpg} &         \includegraphics[width=\linewidth]{tests/over_s_3.jpg} \\
		
		D & 
		\includegraphics[width=\linewidth]{tests/over_m_1.jpg} & \includegraphics[width=\linewidth]{tests/over_m_2.jpg} &         \includegraphics[width=\linewidth]{tests/over_m_3.jpg} \\
		
		E & 
		\includegraphics[width=\linewidth]{tests/size_1.jpg} & \includegraphics[width=\linewidth]{tests/size_2.jpg} &         \includegraphics[width=\linewidth]{tests/size_3.jpg} \\
		
		F & 
		\includegraphics[width=\linewidth]{tests/pass_1.jpg} & \includegraphics[width=\linewidth]{tests/pass_2.jpg} &         \includegraphics[width=\linewidth]{tests/pass_3.jpg} \\
	
	
	\caption{Example test images which have been labelled (A) single underripe, (B) multiple underripe, (C) single overripe, (D) multiple overripe, (E) size difference and small, and (F) pass.}
...
The current under ripe  and over ripe algorithms had very good F1 results of $94.7\%$ and $90.6\%$ respectively, when measured against multiple defect instances whilst the single defect instances achieved $80.1\%$ and $77.1\%$, respectively. Examples of some typical under ripe rejections is shown in Figure \ref{fig:UR_berries}. Development has also included an algorithm for small size fruit defects (berry dimensions are too small), using blob detection and morphological methods generating an accuracy of $>70\%$ when tested using this dataset.
...
Production line speed increase has been considered and would require substantial upgrades to lighting (for example, in the event of doubling the line speed) in order to reduce shutter speeds to appropriate levels and avoid motion blur. Computational capacity would require upgrading with at least the addition of high-performance hardware components such as GPU's in order to perform functions including image threading and processing. Ejector control has the ability to accurately eject at higher rates, given that the punnets could be safely conveyed off the main production line. 
...
\begin{algorithm}
	\caption{Under Ripe Method II}
	\label{alg:underripe}
	\begin{algorithmic}[1]
		\Procedure{AnalyseUnderripe}{$I_{crop}$, $H_{min}$, $H_{max}$, $S_{min}$, $V_{min}$, $U_{max}$, $A_{max}$}       
		\State $Z \gets \emptyset$
		\Comment{initialise empty return set}
		\State $B_{sat} \gets \emptyset$
		\Comment{initialise empty analysis regions}
		\State $B_{val} \gets \emptyset$
		\State $R_{comb} \gets \emptyset$
		\State $(H,S,V) \gets RGBtoHSV(I_{crop})$
		\State $B_{sat} \gets S_{(x, y)}[(x, y) > S_{min}]$
		\Comment{saturation channel region}
		\State $B_{val} \gets V_{(x, y)}[(x, y) > V_{min}]$
		\Comment{value channel region}
		\State $U_{r} \gets B_{sat} \cup B_{val}$
		\Comment{union operation}
		\If{$U_{r} > U_{max}$}
		\State $R_{comb} \gets U_{r}$
		\EndIf
		\For{each $r \in R_{comb}$}
		\If{$r > H_{min}$ and $b < H_{max} $}
		\State $Z \gets Z + r$
		\Comment{accumulate all pixels in range}
		\If{$Z > A_{max}$}
		\State \Return Z
...
\begin{algorithm}
	\caption{Over Ripe Method II}
	\label{alg:overripe}
	\begin{algorithmic}[1]
		\Procedure{AnalyseOverripe}{$R_{berry}$, $V_{low}$, $V_{high}$, $R_{max}$, $A_{max}$}       
		\State $Z \gets \emptyset$
		\Comment{initialise empty return set}
		\State $R_{good} \gets \emptyset$
		\Comment{initialise empty analysis regions}
		\State $R_{cand} \gets \emptyset$
		\State $R_{norm} \gets \emptyset$
		\State $R_{overripe} \gets \emptyset$
		\State $R_{good} \gets V_{(x, y)}[V_{high} > (x, y) > 255]$
		\Comment{light red regions}
		\State $R_{cand} \gets R_{berry} - R_{good}$
		\Comment{potential overripe regions}
		\State $R_{norm} \gets V_{(x, y)}[V_{min} > (x, y) > 255]$
		\Comment{surface normal}
		\State $R_{overripe} \gets R_{cand} \cap R_{norm}$
		\Comment{intersection operation}
		\For{each $r \in R_{overripe}$}
		\If{$r > R_{max}$}
		\State $Z \gets Z + r$
		\Comment{accumulate all pixels in range}
		\If{$Z > A_{max}$}
		\State \Return Z
		\Comment{return area if greater than zero}
		\EndIf
		\EndIf
		\EndFor
		\State \Return 0
...
$R_{cand}$ is the candidate over ripe region which may still contain false-positive pixels. The threshold to find the surface normal ($R_{norm}$) of the berries is then intersected with the candidates to highlight only confident matches as shown in Figures \ref{fig:over_light_diff} and \ref{fig:over_result}. Algorithm \ref{alg:overripe} describes the process of this method.
...
\begin{figure}[h]
	...
	\begin{subfigure}{.45\textwidth}
		\centering
		\includegraphics[width=.6\linewidth]{strip_LED.jpg}
		\caption{}
		\label{fig:strip_LED}
	\end{subfigure}%
	\begin{subfigure}{.45\textwidth}
		\centering
		\includegraphics[width=0.6\linewidth]{panel_LED.jpg}
		\caption{}
		\label{fig:panel_LED}
	\end{subfigure}%
	...
	\begin{subfigure}{.45\textwidth}
		\centering
		\includegraphics[width=.6\linewidth]{ac_LED.jpg}
		\caption{}
		\label{fig:ac_LED}
	\end{subfigure}%
	\begin{subfigure}{.45\textwidth}
		\centering
		\includegraphics[width=0.5\linewidth]{LED_vis.png}
		\caption{}
		\label{fig:LED_vis}
	\end{subfigure}%
	\caption[LED Lighting sources purchased and tested (a) low-power strip LED's (many rows), (b) LED panel, (c) LED's with onboard AC/DC conversion, and (d) high-power COB.]{LED Lighting sources purchased and tested (a) low-power strip LED's (many rows) \textit{Source: https://www.aliexpress.com/item/32955763018.html}, (b) LED panel \textit{Source: https://www.ecolightled.com.au/collections/led-flat-panel}, (c) LED's with onboard AC/DC conversion \textit{Source: https://www.ebay.com.au/itm/100W-Smart-IC-Driver-COB-Lamp-Chip-LED-AC-220-240V-Input-Integrated-for-DIY-/233203041932}, and (d) high-power COB \textit{Source: https://www.vozopca.com/index.php/10w-20w-30w-50w-80w-100w-smd-led-chip-for-high-power-flood-light.html}.}
	\label{}
\end{figure}
...
\begin{figure}[h]
	...
	\end{subfigure}%
	\caption[(a) The effect of poor CRI on colour images making them appear diluted and less vibrant, and (b) Spectral response of different colour temperatures from $2721K$ (warm) through to $6478K$ (cool), with the ideal (daylight/100 CRI) value approximately in the centre of the range.]{(a) The effect of poor CRI on colour images making them appear diluted and less vibrant \textit{Source: https://prolux.cl/que-es-el-color-rendering-index-cri/}, and (b) Spectral response of different colour temperatures from $2721K$ (warm) through to $6478K$ (cool), with the ideal (daylight/100 CRI) value approximately in the centre of the range. \textit{Source: https://indiecinemaacademy.com/complexities-of-bicolor-led-lights-an-extensive-color-analysis/}}
	\label{}
\end{figure}
...
\begin{figure}[h]
	...
	\end{subfigure}%
	\caption[(a) Circuit using reservoir capacitors in order to supply the current required to overload the relatively small LED, and (b) the prototype circuit inside the enclosure whilst triggering LEDs (left and right of image) for testing purposes.]{(a) Circuit using reservoir capacitors in order to supply the current required to overload the relatively small LED \cite{willert}, and (b) the prototype circuit inside the enclosure whilst triggering LEDs (left and right of image) for testing purposes.}
	\label{}
\end{figure}
...
\begin{enumerate}
	...
	\item Continuity - the Phidget\texttrademark is connected to the running application via an temperamental micro USB cable which can become loose over time due to vibration and thermal changes. This is unacceptable as it could cause malfunction of safety systems or prevent them from activating entirely. 
...
\begin{figure}[h]
	...
	\end{subfigure}%
	\caption[Controllino\texttrademark device showing (a) schematic pinout, indicators, and communication ports, and (b) all three controllers mounted on DIN rail during installation.]{Controllino\texttrademark device showing (a) schematic pinout, indicators, and communication ports \textit{Source: https://www.controllino.biz/wp-content/uploads/2019/02/CONTROLLINO-Instruction-Manual-V1.5-2018-12-14.pdf}, and (b) all three controllers mounted on DIN rail during installation.}
	\label{}
\end{figure}
...
OpenCV (Open source computer vision \cite{opencv}) is a library of image functions including reading and writing, converting colourspaces, image processing, and some machine learning algorithms. It is designed to work in unison with the above-mentioned deep learning frameworks, in terms of compatibility and tensor representation. Images are treated as matrices in order to facilitate integration with back-end programs, as well as reducing complexity of image manipulation, due to the ability to index, slice and reshape the matrices. The library does not rely on computational engine in order to perform it's operations, and can be integrated into other frameworks easily.
...
A preliminary investigation into SVM classifier accuracy has been performed with the results showing in Table \ref{tab:svm_results}. The training used only 27 images given the number of input features (pixels) used per image would create exponentially longer training times. For example 27 pre-processed images had a training duration of approximately 50 minutes, while 100 training images increased the training time to more than 5 hours. This exponential difference is due to the SVM process which must load all data into memory before calculating a high dimensional hyperplane. As this was a preliminary test, the increase in experimental efficiency was evaluated to be more beneficial than accurate results using more training data.  

The training images were not included in the testing set of 579 images (test image inference duration is much lower than training), and showed a variety of results when the class region was altered by means of pre-processing, colourspace transformation, and threshold values. For example, the first four tests in the table have similar accuracy results, however, the distribution of $true +$ and $false -/+$ for test four is very different to the previous three. This shows that the training data inputs must be optimised by selecting an appropriate range of pixels correlating to the class required. Therefore, the first step toward improving these results must be to ascertain the best areas of the berry to train the classifier for each class. 
...
\begin{figure}[t]
	...
	\caption[A ConvNet using 2 GPUs with individual processes assigned to each processor.]{A ConvNet using 2 GPUs with individual processes assigned to each processor. \textit{Source: https://d2l.ai/chapter\_computational-performance/multiple-gpus.html}}
	\label{fig:convnet1}
\end{figure}%
...
\begin{figure}[h]
	\centering
	\includegraphics[width=0.7\linewidth]{covnet2.jpg}
	\caption[ConvNet implementation used for learning the MNIST handwritten digit dataset.]{ConvNet implementation used for learning the MNIST handwritten digit dataset. \textit{Source: https://www.researchgate.net/figure/The-arquitecture-of-one-column-of-the-convolutional-neural-network-that-achieved-the-best\_fig4\_283831532}}
	\label{fig:convnet2}
\end{figure}%
...
\begin{wrapfigure}{r}{0.5\textwidth}
	...
	\end{center}
	\caption[Examples of first layer convolutional filters taken from Stanford's CS231N course.]{Examples of first layer convolutional filters taken from Stanford's CS231N course \cite{andrej}.}
...
Using a combination of horizontal and vertical flipping, images were compiled for training and listed in Table \ref{tab:data}. The number of testing images for each network was constant at 500, however, due to the absence of training data, three augmentations were performed, namely, horizontal, vertical, and diagonal flipping in order to increase the number of samples. 
...
Accuracy for all networks began to plateau after the maximum listed epoch for each architecture, likely due to the small amount of data. 
...
\begin{figure}[h]
	...
	\caption[A residual neuron used in ResNet architecture showing the identity function skipping a connection.]{A residual neuron used in ResNet architecture showing the identity function skipping a connection. \textit{Source: https://mc.ai/resnet-architecture-explained/}}
	\label{fig:residual}
\end{figure}
...
\begin{wrapfigure}{r}{0.5\textwidth}
	...
	\end{center}
	\caption[A residual network in comparison to VGG-$19$ network and a $34$-layer non-residual network.]{A residual network in comparison to VGG-$19$ network and a $34$-layer non-residual network. \textit{Source: https://towardsdatascience.com/an-overview-of-resnet-and-its-variants-5281e2f56035}}
	\label{fig:resnet_full}
\end{wrapfigure} 


Figure \ref{fig:residual} shows a graphical representation of a connection in a Residual Network architecture whilst Figure \ref{fig:resnet_full} shows the full architecture of a $36$-layer ResNet in comparison to a 'plain' 36-layer network and VGG-19. 
...
where $x$ and $y$ are the input and outputs and the function $\mathcal{F}(x.\{W_i\})$ is the learned mapping. This equation holds for all layers where the dimensions of $x$ and $\mathcal{F}$ are the same. If they are different, for instance when reducing channels, the function is modified slightly to add a linearly projected matching parameter $W_s$:
\begin{equation}
...
The authors of this paper compared the residual networks (of differing depths up to 1000 layers) with 'plain' networks, VGG-16 \cite{simonyan}, GoogLeNet \cite{szegedy}, PReLU-net \cite{he2}, and BN-inception \cite{ioffe} finding that ResNet's ability performed at up to eight times deeper than VGG-16, but with less complexity. The results outperformed other networks when classifying ImageNet and CIFAR-10, as well as a $28\%$ improvement on COCO object detection. An ensemble of ResNets achieved $3.37\%$ error rate winning first prize in the ILSVRC 2015 classification task.
...
%first page
\begin{minipage}[b]{0.9\linewidth}
	\includepdf[pages=1,scale=0.9,linktodoc=true]{appendix/GUI.pdf}
\end{minipage}

%rest of pages
\includepdf[pages=2-,scale=0.9]{appendix/GUI.pdf}
